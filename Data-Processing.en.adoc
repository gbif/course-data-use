...
Usually, when we want to use the data, we will have to process the data to only include those data whose correctness and consistency are within the thresholds set by the analysis we want to perform.  
For instance, if we are studying plant biogeography in Indonesia, and want to do to a specific analysis for only one of the islands within the archipelago, then the download is no longer fit for purpose and we will have to process our original dataset download. 
...
In the Indonesian example, an appropriate question might be - Have localities been correctly georeferenced?
...
In the Indonesian example, an appropriate question might be - Does the uncertainty in the coordinate allow for the occurrence record to not be on the island?
...
Here are some additional things you might want to do to your data. 
These things are little bit more complex and involve more judgement calls, so I leave them out of the main cleaning pipeline script. 
